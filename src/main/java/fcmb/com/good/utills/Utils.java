    ...
    public static Object getData(Row row, String key, Map<String, Integer> map) {
        ...
        if (map.containsKey(key)) {
            ...
            if(dateFields().contains(key)){
                if (row.getCell(cellIndex).getCellType().equals(CellType.STRING)) {
                    return row.getCell(cellIndex).getStringCellValue();
                }
                double result =row.getCell(cellIndex).getNumericCellValue();
                Date javaDate= DateUtil.getJavaDate(result);
                SimpleDateFormat ft = new SimpleDateFormat("yyyy-MM-dd");
                return ft.format(javaDate);
            }
